...
var rasca_1 = document.getElementById("rasca_1"), rascaCanvas_1 = rasca_1.getContext('2d'), brushRadius = (rasca_1.width / 100) * 5, img_1 = new Image();
var toques_1 = 0;
...
/* Inicio duplicado elementos base */

var rasca_2 = document.getElementById("rasca_2"), rascaCanvas_2 = rasca_2.getContext('2d'), brushRadius = (rasca_2.width / 100) * 5, img_2 = new Image();
var toques_2 = 0;
/* parametros de la imagen que carga para tapar el rasca */
img_2.loc = 'https://s3-us-west-2.amazonaws.com/s.cdpn.io/4273/';
img_2.filename = 'calgary-bridge-2013.jpg';
if (window.devicePixelRatio >= 2) {
	var nameParts = img_2.filename.split('.');
	img_2.src = img_2.loc + nameParts[0]+"-2x"+"."+nameParts[1];
    
} else {
	img_2.src = img_2.loc + img_2.filename;
    
}

/*Fin dpulicado elementos base*/
...
function getBrushPos(xRef, yRef) {
    ...
    return {
	  x: Math.floor((xRef-rascaRect_1.left)/(rascaRect_1.right-rascaRect_1.left)*rasca_1.width),
	  y: Math.floor((yRef-rascaRect_1.top)/(rascaRect_1.bottom-rascaRect_1.top)*rasca_1.height)
    };
}
...
rasca_1.addEventListener("mousemove", function(e) {
...
rasca_1.addEventListener("touchmove", function(e) {
    e.preventDefault();
...
function getBrushPos_2(xRef, yRef) {
    ...
    return {
	  x: Math.floor((xRef-rascaRect_2.left)/(rascaRect_2.right-rascaRect_2.left)*rasca_2.width),
	  y: Math.floor((yRef-rascaRect_2.top)/(rascaRect_2.bottom-rascaRect_2.top)*rasca_2.height)
    };
}
// x e y de cada canvas uno por rasca 
...
//controla si pulsa el mouse una por cada rasca
rasca_2.addEventListener("mousemove", function(e) {
...
//controla si si pulsa el dedo una por cada rasca
rasca_2.addEventListener("touchmove", function(e) {
    e.preventDefault();
    var touch = e.targetTouches[0];
